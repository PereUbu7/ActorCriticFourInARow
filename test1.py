...
class ConvolutionalNetwork():
    def __init__(self, batch_size=1, scope="conv_net"):
        with tf.variable_scope(scope):
            ...
            #self.filter3 = tf.Variable(tf.random_normal([2, 2, 40, 100]), name="filter3")
            ...
            self.outLayer = tf.nn.relu(self.conv2)
...
class PolicyEstimator():
    ...
    def __init__(self, entropyFactor=0.1, shared_layers=None, batch_size=1, scope="policy_estimator"):
        with tf.variable_scope(scope):
            ...
            self.input = tf.concat([self.board_flat, self.input], 1)
            ...
            self.l2_dropout = tf.contrib.layers.dropout(
                self.l2,
                keep_prob=0.5,
            )

            self.mu = tf.contrib.layers.fully_connected(
                inputs=self.l2_dropout,
...
class ValueEstimator():
    ...
    def __init__(self, learning_rate=0.1, shared_layers=None, batch_size=1, scope="value_estimator"):
        with tf.variable_scope(scope):
            if shared_layers is not None:
                self.board = shared_layers.board
                self.input = shared_layers.outLayer
        ...
        for t in itertools.count():
            ...
            if t > 0:
                ...
                if batch_pos == batch_size:
                    ...
                    loss = trainer.update(batch_board, batch_player, batch_td_target, batch_td_error, batch_action, batch_avaliableColumns)
                    loss = loss[0][0]
                    batch_pos = 0
...
conv_net = ConvolutionalNetwork(batch_size=batch_size)
policy_estimator = PolicyEstimator(entropyFactor=1e-2, shared_layers=conv_net, batch_size=batch_size)
value_estimator = ValueEstimator(learning_rate=0.001, shared_layers=conv_net, batch_size=batch_size)
trainer = Trainer(learning_rate=1e-3, policy=policy_estimator, policyLossFactor=1, value=value_estimator, valueLossFactor=1e-0)
